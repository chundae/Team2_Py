...
from lstm_model import preprocess_data, build_lstm_model, predict_with_lstm
import pandas as pd
...
# 메인 함수
# 메인 함수
def main(file_path, start_date, end_date, currency, frequency, lstm_predict):
    ...
    if lstm_predict.upper() == 'Y':
        X, y, scaler = preprocess_data(filtered_data, currency_columns[0], n_steps=60)
        model = build_lstm_model(X.shape[1:])
        model.fit(X, y, epochs=100, batch_size=32)

        predictions = predict_with_lstm(model, filtered_data[currency_columns[0]], scaler,
                                        n_steps=60)  # n_step 예측 기간 설정
        plot_single_series(pd.DataFrame(predictions, index=filtered_data.index[-len(predictions):]), column=0,
                           title='LSTM 예측 환율 변동', xlabel='날짜', ylabel='환율')
...
if __name__ == "__main__":
    ...
    LSTM_YN = input("데이터 시계열 예측 Y | N : ")

    main(file_path, start_date, end_date, currency, frequency, LSTM_YN)
